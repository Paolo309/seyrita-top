...
# TODO Add back if trst is used
# set_max_delay -from [get_ports jtag_trst_ni] $JTAG_TCK
# set_false_path -hold -from [get_ports jtag_trst_ni]

# TODO should check whether to also separate rtc clock
# >> report_clock_networks -clock [get_clocks *rtc*]
...
# HyperBus is present but inactive: ignoring its internal clocks
set_false_path -through [get_cells -hier -filter {NAME =~ *i_hyperbus_wrap*}]

# sys_resetn is asynchronous, used only for reeset generation # TODO not needed in Cheshire: check if still needed here
