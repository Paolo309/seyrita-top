...
make chim-xilinx-program-$BOARD_CLASS CHIM_XILINX_HWS_URL=boardberg.ee.ethz.ch:$TCP_PORT

echo "Sending reset ..."
make chim-xilinx-reset-$BOARD_CLASS CHIM_XILINX_HWS_URL=boardberg.ee.ethz.ch:$TCP_PORT
...
# sed -i "s/target extended-remote localhost:3333/target extended-remote boardberg.ee.ethz.ch:$GDB_PORT/" .gdbinit
...
scp .gdbinit msc25h20@boardberg.ee.ethz.ch:$WORKDIR/
scp utils/fpga/openocd.*.tcl msc25h20@boardberg.ee.ethz.ch:$WORKDIR/
scp $CHISDK_ROOT/build-ASIC/bin/test_mxita_app msc25h20@boardberg.ee.ethz.ch:$WORKDIR/

OPENOCD_PID=$(ssh msc25h20@boardberg.ee.ethz.ch "sh -c 'cd $WORKDIR && nohup openocd -f msc25h20-$BOARD-openocd.tcl -f openocd.$BOARD_CLASS.tcl >/dev/null 2>&1 & echo \$!'")
...
ssh msc25h20@boardberg.ee.ethz.ch "cd $WORKDIR && gdb-multiarch -iex 'set auto-load safe-path /' test_mxita_app" | tee gdb_output.log
...
ssh msc25h20@boardberg.ee.ethz.ch "kill $OPENOCD_PID"

rm -rf $WORKDIR/*

echo "Releasing board $BOARD ..."
ssh msc25h20@boardberg.ee.ethz.ch "bash -lc 'fpga release -b $BOARD'"
